 ...
 ***************************************************************************************
 */
package com.arjim.rebot.proxy.impl;
...
import com.arjim.rebot.model.RobotMessage;
import com.arjim.rebot.model.RobotMessageArticle;
import com.arjim.rebot.proxy.RebotProxy;
...
public class TLRebotProxy implements RebotProxy {
	...
	public MessageWrapper botMessageReply(String user, String content) {
		...
		String message = "";
		try {
			Document doc = Jsoup.connect(apiUrl).timeout(62000).data("key", key).data("userid", user)
					.data("info", content).post();
			...
			case 100000:
				...
				break;
			case 200000:
				message = msg.getText() + "<a href='" + msg.getUrl() + "' target='_blank;'>" + msg.getUrl()
						+ "</a></br>";
				break;
			case 302000:
				...
				for (RobotMessageArticle a : sublist) {
					// message+="a("+a.getDetailurl()+")["+a.getArticle()+"] <br><br>";
					// //layim聊天窗口回复内容
			...
			case 308000:
				...
				for (RobotMessageArticle a : sublistOne) {
					// message+="菜名："+a.getName()+"<br>用料："+a.getInfo()+"
					// <br>详情地址:a("+a.getDetailurl()+")["+a.getDetailurl()+"]
					// <br><br>";//layim聊天窗口回复内容
					message += "菜名：" + a.getName() + "<br>用料：" + a.getInfo() + "  <br>详情地址:<a href='" + a.getDetailurl()
							+ "' target='_blank;'>" + a.getDetailurl() + "</a>      </br>";
				}
				break;
			default:
				break;
			...
			return new MessageWrapper(MessageWrapper.MessageProtocol.REPLY, Constants.ImserverConfig.REBOT_SESSIONID,
					user, result.build());
		} catch (Exception e) {
			e.printStackTrace();
		}
		return new MessageWrapper(MessageWrapper.MessageProtocol.REPLY, Constants.ImserverConfig.REBOT_SESSIONID, user,
				null);
	}
