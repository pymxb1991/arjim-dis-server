package com.arjim.webserver.util;

import com.arjim.server.model.ServerSslKeyStore;
...
public class FileUtil {
	...
	/**
	 * 验证文件是否允许上传 首先检查文件后缀是否有效，通过之后则检查文件编码是否正确
	 *
	 * @param file    文件
	 ...
	 * @return
	 */
	public static ServerSslKeyStore getSslProInfo() {


		String path = "D://project//ecplice//ide-workspace//arjim-server//src//main//resources//keystore.jks";
		String password="";
		String type="JKS";
		try {
			Resource resource = new ClassPathResource("/resource.properties");
			Properties props = PropertiesLoaderUtils.loadProperties(resource);
			//path = (String) props.get("server.ssl.key-store");
			password = (String) props.get("key-store-password");
			type = (String) props.get("server.ssl.key-store-type");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return new ServerSslKeyStore(type,password,path);
	}
}
